...
- [FINAL_PROMPT]: Write a descriptive video paragraph (4-8 sentences). Describe the final scene as if narrating a film — NOT instructions, NOT a checklist. Only describe what the viewer sees after all changes from [EXECUTE] are applied. Describe the source subjects in their new setting as one cohesive scene — do NOT narrate reference image contents as if they were separate or independent.
...
class Qwen35PromptEnhancer:
    ...
    def enhance(self, model, mmproj, task_type, variant, prompt,
        ...
        sp, ut, order = _route(effective_task, prompt, len(ref))
